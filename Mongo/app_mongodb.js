...
server.post("/get", (request, response) => {
  ...
  MongoClient.connect("mongodb://XXXXXXXXX") // => Just create database on https://realm.mongodb.com/ for learning purpose
    .then((client) => {
    ...
    .then((client) => {
      ...
      db.collection("employee")
        ...
        .then((result) => {
          ...
          else response.json({ status: false, msg: "Not Saved !" });
        })
        .catch((err) => {
          console.log(err);
